...
if (isset($_SESSION[$session_key]) && !empty($_SESSION[$session_key])) {
    ...
    foreach ($template_configs as $config) {
        ...
        // $template_name = $config['template_name']; // Bu bilgi artık frontend'de kullanılmayacak
        ...
        ";
        // $template_name frontend'de kullanılmayacağı için SELECT'ten de kaldırılabilir, ancak şimdilik kalsın
        // Eğer source_template_name frontend'den tamamen kaldırıldıysa, SELECT'ten de kaldırılabilir.
        // Şimdilik $question['source_template_name'] frontend'de gösterilmediği için PHP tarafında kalması sorun teşkil etmez.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $total_score = 0; // Bu skor backend'de hesaplanmaya devam ediyor, sadece frontend'de gösterilmeyecek
        // $total_multiple_choice = 0; // Frontend'de gösterilmeyeceği için bu değişkene gerek kalmayabilir, ama backend loglama/analiz için tutulabilir.
                                    // Şimdilik backend'de kalabilir, ileride tamamen kaldırılabilir.
        
        // Session'daki sorular ile POST'taki yanıtları karşılaştır
        ...
        foreach ($multiple_choice_questions as $question) {
            ...
            if (isset($posted_answers[$template_question_id]) && 
                ...
                $is_correct_score = 0; // Skor artık 0-100 değil, sadece doğru/yanlış için tutulabilir (örneğin 1 veya 0) ya da direkt $total_score'a eklenebilir
                
                if (isset($correct_options[$template_question_id]) && 
                    $option_id == $correct_options[$template_question_id]) {
                    $total_score++; // Ya da soru başına farklı puanlama varsa o eklenebilir
                    $is_correct_score = 100; // DB'deki answer_score alanı için hala 100 kullanılabilir. Frontend'de gösterilmeyecek.
                }
                ...
                $stmt = $db->prepare("
                    ...
                    VALUES (?, ?, ?, ?, ?)
                ");
                // answer_text için $question['question_text'] yerine, seçilen opsiyonun metni veya boş string kaydedilebilir.
                // Şimdiki haliyle sorunun metnini kaydediyor, bu da bir tercih.
                $stmt->execute([$application_id, $template_question_id, $option_id, $question['question_text'], $is_correct_score]);
                
            } else {
                // Yanıtlanmamış soru - 0 puan olarak kaydet
                $stmt = $db->prepare("
                    INSERT INTO application_answers (application_id, question_id, option_id, answer_text, answer_score) 
                    VALUES (?, ?, ?, ?, ?)
                ");
        ...
        foreach ($open_ended_questions as $question) {
            ...
            // Açık uçlu soru için cevap yoksa boş olarak kaydet
            if (!$has_answer && $question['question_type'] !== 'multiple_choice') { // Sadece açık uçlular için "yanıtlanmadı"
                $answer_text_to_save = '(Yanıtlanmadı)';
            } else {
                $answer_text_to_save = $answer_text;
            }
            
            // Eğer hem metin hem dosya varsa, metni önceliklendir ya da birleştir. Şimdiki haliyle metin ve dosya ayrı kaydediliyor.
            // Açık uçlu soruların skoru manuel olarak atanacağı için answer_score genellikle 0 olarak başlar.
            $stmt = $db->prepare("
        ...
        // $total_score, çoktan seçmeli sorulardan gelen skoru applications tablosuna kaydeder.
        // Bu skor İK tarafından görülebilir, ancak adaya gösterilmeyecek.
        ...
        // error_log("Session Questions: " . print_r(array_map(function($q) { return $q['id']; }, $questions), true));
        // error_log("Posted Answer IDs: " . print_r(array_keys($posted_answers ?? []), true));
    }
}
?>
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Allow text selection for textareas and inputs only */
        * {
            user-select: none;
        }
        ...
        /* .question-badges and .question-source removed or styles not needed if elements are removed */
        ...
        /* score-info and related styles removed */
...
<body>
                                ...
                                <div class="question-box" style="--animation-order: <?= $index ?>">
                                    <div class="question-header">
                                        ...
                                        <div class="question-content">
                                            ...
                                            <!-- Soru kategorisi ve tipi buradan kaldırıldı -->
                                        </div>
                                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Prevent text selection for most elements but allow for textareas
            ...
            // Prevent copy/cut but allow paste (Ctrl+V)
            document.addEventListener('copy', function(e) {
                const selection = window.getSelection().toString();
                if (selection.length > 0) { // Only prevent if there's an actual selection
                    const activeElement = document.activeElement;
                     // Allow copy from input/textarea for user's own input
            ...
            document.addEventListener('cut', function(e) {
                 const activeElement = document.activeElement;
                 // Allow cut from input/textarea for user's own input
                if (activeElement.tagName !== 'INPUT' && activeElement.tagName !== 'TEXTAREA') {
                    e.preventDefault();
                    return false;
                }
            ...
            document.addEventListener('paste', function(e) {
                // Allow paste only in textareas and input fields
            ...
            // Prevent keyboard shortcuts except Ctrl+V
            document.addEventListener('keydown', function(e) {
                // Allow Ctrl+V for paste
                if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'v') {
                    const target = e.target;
                    if (target.tagName === 'TEXTAREA' || (target.tagName === 'INPUT' && (target.type === 'text' || target.type === 'email' || target.type === 'search'))) {
                        return true; // Allow Ctrl+V in textareas and inputs
                    }
                }
                
                // Prevent other Ctrl combinations if not in input/textarea
                const activeElement = document.activeElement;
                if (!(activeElement.tagName === 'INPUT' || activeElement.tagName === 'TEXTAREA')) {
                ...
                } else { // If in input/textarea, allow Ctrl+A (select all) and Ctrl+S (which browser might handle for save page)
                     if ((e.ctrlKey || e.metaKey) && (e.key.toLowerCase() === 'c' || e.key.toLowerCase() === 'x' )) {
                        // Copy/cut from inputs are handled by their specific event listeners
                     } else if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'u') { // Block Ctrl+U (view source) always
                        e.preventDefault();
                        return false;
                     }
                }
            ...
            document.addEventListener('contextmenu', function(e) {
                const target = e.target;
                 // Allow context menu for text inputs and textareas for cut/copy/paste operations by user
                if (target.tagName === 'TEXTAREA' || (target.tagName === 'INPUT' && (target.type === 'text' || target.type === 'email' || target.type === 'search'))) {
                    return true;
                }
                e.preventDefault();
            ...
            // Enhanced file upload UI with better feedback
            const fileInputs = document.querySelectorAll('input[type="file"]');
            fileInputs.forEach(input => {
                input.addEventListener('change', function(e) {
                    const file = e.target.files[0];
                    const fileLabel = this.nextElementSibling; // Assuming label is the next sibling
                    const icon = fileLabel.querySelector('i');
                    const nameSpan = fileLabel.querySelector('span');
                    const fileInfoDiv = fileLabel.nextElementSibling; // Assuming file-info is sibling after label

                    if (file) {
                        const fileName = file.name.length > 25 ? file.name.substring(0, 22) + '...' : file.name;
                        if (nameSpan) nameSpan.textContent = fileName;
                        if (icon) icon.className = 'bi bi-file-earmark-pdf-fill'; // Filled icon
                        if (fileLabel) {
                        ...
                        if (fileInfoDiv) {
                            ...
                            fileInfoDiv.style.color = 'var(--text-dark)'; // Make it more visible
                        }
            ...
            // Enhanced timer with session-based remaining time - PHP'den gelen kalan süre ile başlat
            const totalTimeInSeconds = <?= $remaining_seconds ?>; // PHP'den gelen kalan süre
            ...
            if (timerElement && countdownElement && quizForm) {
                if (timeLeft <= 0 && '<?= $_SERVER['REQUEST_METHOD'] ?>' === 'GET') { // Only auto-submit if it's a GET request and time is up
                     // This means page loaded and time was already up, handled by PHP's $timeout_expired
                } else if (timeLeft <=0 && '<?= $_SERVER['REQUEST_METHOD'] ?>' === 'POST') {
                    // Form already submitted, do nothing with timer
                }
                else if (timeLeft > 0) {
            ...
            function updateTimer() {
                if (timeLeft <= 0) {
                    ...
                    if (!quizForm.dataset.submitted && '<?= $_SERVER['REQUEST_METHOD'] ?>' === 'GET') { // Ensure form hasn't been submitted by user
                       autoSubmitForm();
                    }
                    return;
                }
                ...
                if (timeLeft <= 600 && timeLeft > 300) { // 10 dakika
                    countdownElement.classList.add('warning');
                    countdownElement.classList.remove('danger');
                } else if (timeLeft <= 300 && timeLeft > 60) { // 5 dakika
                    countdownElement.classList.remove('warning');
                    countdownElement.classList.add('danger');
                } else if (timeLeft <= 60) { // 1 dakika
                    countdownElement.classList.add('danger');
                    if (timeLeft % 2 === 0) {
            ...
            function autoSubmitForm() {
                if (quizForm.dataset.submitted) return; // Prevent re-submission
                quizForm.dataset.submitted = 'true'; // Mark as submitted by auto-submit
                ...
                }, 2500); // Give a bit more time for user to see the message
            }
            
            // Enhanced option card interactions
            ...
            optionCards.forEach((card) => {
                card.addEventListener('click', function() {
                    ...
                    setTimeout(() => {
                        if (this.classList.contains('selected')) { // Re-check to avoid race condition if user clicks fast
            ...
            // Initially check for selected options (e.g. if page reloads with form data)
            document.querySelectorAll('input[type="radio"]:checked').forEach(radio => {
                const card = radio.closest('.option-card');
                if (card) {
                    card.classList.add('selected');
                    card.style.transform = 'translateX(8px)'; // Apply selected style
                }
            });
            
            if (quizForm) {
                quizForm.addEventListener('submit', function(e) {
                    if (this.dataset.submitted === 'true' && this.dataset.submittedBy !== 'user') {
                        // If auto-submitted, allow it. If user tries to submit again after auto-submit, could be an issue.
                        // This logic can be refined. For now, if submitted, it's submitted.
                    }
                    this.dataset.submitted = 'true';
                    this.dataset.submittedBy = 'user'; // Mark as submitted by user action
            ...
            window.addEventListener('beforeunload', function(e) {
                if (quizForm && quizForm.dataset.submitted === 'true') {
                    return; // Allow unload if form is already submitted
                }
                
                // Check if there are any questions and if time is still left
                const hasQuestions = <?= !empty($questions) ? 'true' : 'false' ?>;
                if (hasQuestions && timeLeft > 0) {
            ...
            formElementsForBackup.forEach(el => {
                el.addEventListener('input', () => { // 'input' for textareas, 'change' for radios
            ...
            if (backupData && !('<?= $success ?>' === '1' || '<?= $timeout_expired ?>' === '1')) { // Don't restore if quiz completed or timed out
                try {
            ...
            // Clear backup on successful submission or timeout
            if (quizForm) {
                quizForm.addEventListener('submit', function() {
                    localStorage.removeItem(`quiz_backup_<?= $application_id ?>_<?= $job_id ?>`);
                });
            }
             if ('<?= $timeout_expired ?>' === '1') {
                 localStorage.removeItem(`quiz_backup_<?= $application_id ?>_<?= $job_id ?>`);
            }
                ...
                if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
                    ...
                    if (focusedElement && focusedElement.closest('.option-card')) {
                        ...
                        if (optionsInputs[nextIndex]) {
                            ...
                            // optionsInputs[nextIndex].click(); // This might be too aggressive, focus is enough
                            // Trigger click on the parent label of the radio for consistent behavior
            ...
            function markQuestionVisualCompletion(questionBox, isComplete) {
                ...
                if (isComplete) {
                    questionBox.style.borderColor = 'var(--success)'; // Subtle border change
                    if (questionNumberEl && !questionNumberEl.dataset.completed) {
                        ...
                        questionNumberEl.innerHTML = '<i class="bi bi-check-lg"></i>'; // Larger check
                        questionNumberEl.dataset.completed = 'true';
                    }
                } else {
                     questionBox.style.borderColor = 'var(--border-light)'; // Reset border
                    if (questionNumberEl && questionNumberEl.dataset.completed) {
                        // Revert to number if needed, or keep check if once answered means "touched"
                        // For simplicity, let's keep the check if it was ever completed.
                        // To revert:
                        // questionNumberEl.style.background = 'linear-gradient(135deg, var(--primary) 0%, var(--primary-dark) 100%)';
                        // questionNumberEl.innerHTML = questionNumberEl.dataset.originalNumber; // Store original number if reverting
                        // delete questionNumberEl.dataset.completed;
                    }
                }
            }
            ...
            document.querySelectorAll('.question-box').forEach(qBox => {
                // Store original number for potential revert
                const qNumEl = qBox.querySelector('.question-number');
                if (qNumEl) qNumEl.dataset.originalNumber = qNumEl.textContent;

                qBox.addEventListener('change', () => checkQuestionCompletion(qBox)); // For radio, file
                const textarea = qBox.querySelector('textarea');
                if (textarea) {
                    textarea.addEventListener('input', () => checkQuestionCompletion(qBox)); // For textarea
                }
                checkQuestionCompletion(qBox); // Initial check on load
            });
            
            console.log('Quiz initialized with enhanced features and session-based timer.');
        });
    </script>
</body>
</html>
...
<?php 
if (isset($db) && $db->inTransaction()) { // Safety check if transaction is still open due to an early exit before commit/rollback
    $db->rollBack();
}
